...
function WhiteCheckIcon() {
  return (
    ...
    >
      <path
        d="M33.0689 10.6992L15.1022 28.6659L6.93555 20.4992"
        stroke="white"
        strokeWidth="4"
        strokeLinecap="round"
        strokeLinejoin="round"
...
function ReferandEarn() {
  ...
  const referralLink = `https://orbissocial.app.link/${code}`;
  ...
  const generateQR = async () => {
    try {
      setQR(await QRCode.toDataURL(referralLink));
    } catch (err) {
      console.error(err);
    }
  };
  ...
  return (
    <article className="-order-1 lg:order-1 border rounded-lg border-[#E4E7EC] p-6 py-8 space-y-4">
      <div className="w-full text-center max-w-md mx-auto">
        <p className="font-bold lg:text-lg uppercase">
          Copie e cole este link em seu navegador para baixar nosso aplicativo
        </p>
      </div>
      <div className="p-4 bg-green-600 bg-opacity-5 flex justify-between items-center rounded-md">
        <div className="w-[95%] relative text-[14px] pl-4 before:content-[''] before:h-full before:w-[2px] before:rounded-lg before:bg-black before:absolute before:left-0">
          <p>URL para copiar</p>
          <p className="font-medium block w-[85%] truncate">{referralLink}</p>
        </div>
        <div className="w-[40px]">
          {linkCopied ? (
            <div className="bg-green-600 h-[20px] w-[20px] flex items-center justify-center rounded-full p-[2px]">
              <WhiteCheckIcon />
            </div>
          ) : (
      ...
      <div className="flex items-center justify-between">
        <div className="w-full h-[1px] bg-[#979BA3]"></div>
        <p className="whitespace-nowrap mx-4 text-[#979BA3] text-xs lg:text-sm">
          or
        </p>
        <div className="w-full h-[1px] bg-[#979BA3]"></div>
      </div>
      ...
      <div className="flex items-center justify-center">
        <div className="flex flex-col items-center gap-8">
          <div className="text-center flex flex-col items-center gap-2">
            <div className="text-heading bg-secondary rounded-full w-fit p-4">
              <QRIcon />
            </div>
            <div className="max-w-md mx-auto">
              <p className="font-bold lg:text-lg uppercase">
                Você também pode escanear este código QR para copiar o link
              </p>
            </div>
          </div>
          {qr ? (
            <Image
              className="bg-[#F8F8F9] border border-[#E4E7EC] rounded-lg max-w-[50vw] aspect-square object-contain"
              src={qr}
              alt="qr-code"
              height="200"
              width="200"
...
export default function Referral() {
  return (
    <section className="pt-10 md:pt-20 relative z-10 max-w-6xl px-4 lg:px-0 lg:mx-auto py-6">
      <ReferandEarn />
    </section>
  );
}
