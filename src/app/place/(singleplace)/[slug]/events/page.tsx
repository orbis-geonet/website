...
const Events = async ({ params }: { params: { slug: string } }) => {
  const data = await getData(`places/slug/${params.slug}/events`);
  ...
  return (
    <>
      ...
      <section className="space-y-8 max-w-6xl mx-auto">
        ...
        {events.length === 0 ? (
          <Noexistence message="Não há nenhuma postagem" />
        ) : (
          events.length === 20 && (
            <ClientEventsFeed endpoint={`places/slug/${params.slug}/events`} />
          )
        )}
      </section>
    </>
